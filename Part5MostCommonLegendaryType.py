...
for line in pokedex:
    ...
    if poke.Legendary:
        if poke.Type1 not in legendaryType:
            legendaryType[poke.Type1]=0
        legendaryType[poke.Type1]+=1

        if poke.Type2 not in legendaryType:
            if poke.Type2=="":
                legendaryType["None"]=0
            else:
                legendaryType[poke.Type2]=0
        if poke.Type2=="":
            legendaryType["None"]+=1
        else:
            legendaryType[poke.Type2]+=1
print(legendaryType)
for x in legendaryType:
    if legendaryType[x]>highest:
        highest=legendaryType[x]
        highestType=x
